...
if (currentScreen.orientation && currentScreen.orientation.lock) {
  currentScreen.orientation.lock('landscape')
    .then(function() {
      console.log('Đã xoay màn hình sang chế độ ngang');
    })
    .catch(function(error) {
      console.error('Khóa hướng màn hình không thành công:', error);
    });
} else {
  console.log('Trình duyệt không hỗ trợ Screen Orientation');
}

  
  return (
    <div id='root' className="background " style={{ backgroundImage: `url(${backgroundImage})`, height: 3405 * ratio, width: 1920 * ratio, }} >
      <img src={title} className='absolute' alt="title" style={{ height: 234 * ratio, width: 1607 * ratio, top: 300 * ratio, left: 120 * ratio }} />
      <p className='absolute text-center font-bold text-white' style={{ fontSize: 36 * ratio, top: 700 * ratio, left: 200 * ratio }} >GameFi Tap to Earn NFT, with a completely unique gameplay and money-making method,<br />
      ...
      <p className='absolute font-bold text-white' style={{ fontSize: 100 * ratio, top: 1990 * ratio, left: 710 * ratio }} >START - IN</p>
      ...
      <p className='absolute font-bold text-white' style={{ fontSize: 60 * ratio, top: 2730 * ratio, left: 670 * ratio }} >VESTING SCHEDULE</p>
